...
int main() {
    ...
    cin >> n >> m;
    for (ll i = 1; i <= m; i++) {
        ll a, b, capacidad, costo;
        cin >> a >> b >> capacidad >> costo;
        G.addEdge(a, b, capacidad, costo, true);
    }
    pair<ll, ll> pa = G.minCostFlow(1, 7, 8);
    cout << pa.first << " adasd " << pa.second << endl;
    for (ll i = 0; i < G.adj[1].size(); i++) {
        cout << G.adj[1][i].to << " asd " << G.adj[1][i].flow << " qwe " << G.adj[1][i].cap
             << " ert " << G.adj[1][i].cost << endl;
    }
    return 0;
}
